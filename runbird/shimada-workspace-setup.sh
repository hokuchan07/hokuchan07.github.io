...
# 1. ディレクトリ作成（人事/契約/経営戦略/資産運用=社長専用、共有=社員参照可）
mkdir -p "$WORKSPACE/人事" "$WORKSPACE/契約" "$WORKSPACE/経営戦略" "$WORKSPACE/資産運用" "$WORKSPACE/ナレッジ" "$WORKSPACE/.cursor/rules" "$WORKSPACE/.scripts"
echo "[OK] フォルダ構造作成（人事/ 契約/ 経営戦略/ 資産運用/ ナレッジ/）"
...
| 「経営戦略に保存」「戦略に保存」「M&A」「買収」「資金調達」（経営機密文脈） | `経営戦略/{YYYYMMDD}_{要約タイトル}.md` | **社長手元のみ**（GitHub非公開） |
| 「資産運用に保存」「資産に保存」「節税」「税理士」（プライベート財務文脈） | `資産運用/{YYYYMMDD}_{要約タイトル}.md` | **社長手元のみ**（GitHub非公開） |
| 「共有して」「みんなに展開」「クローンに追加」 | `共有/{領域}/{YYYYMMDD}_{要約タイトル}.md` | **全社員参照可**（GitHub経由でクローンが回答に使用） |

**共有判断ガイド:** https://hokuchan07.github.io/runbird/shimada-clone-routing-guide.html

「共有して」と社長が明示的に言った場合のみ、共有/ に保存する。それ以外は社長手元のみのフォルダに保存（=GitHub非公開）。判断に迷ったらまず社長専用フォルダに保存し、後から必要なら共有/にコピーする運用が安全（共有→社長専用は不可逆、社長専用→共有は後から可能）。
...
- `経営戦略/` … M&A・資金調達・株主・取締役会等の経営機密（社長手元のみ）
- `資産運用/` … 個人金融・節税・税理士相談等のプライベート財務（社長手元のみ）
...
- **共有判断ガイド（必読・3つの軸）：https://hokuchan07.github.io/runbird/shimada-clone-routing-guide.html**
